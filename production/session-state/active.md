...
- **Art track (background art-director)**: 8 MVP asset specs committed (`856d2c0`, `design/assets/specs/`) — player ship, 3 kaiju (+SOFTENED/BROKEN states), 8 weapons, enemy bullets, material icons, break-VFX; all PENDING (await API key). **GAPS**: master manifest + HUD spec not done, agent stopped short (I committed its work so it wasn't lost).
